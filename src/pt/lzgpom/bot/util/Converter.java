...
public class Converter 
{
	public static String createFinalScoresMessage(TierList tierList)
	{
		...
		out.append(String.format("%s %s - %d%n", Utils.getReactionInPos(place), scores.get(0).getPerson().getName(), points));
		...
		for(int i = 1; i < scores.size(); i++)
		{
			...
			out.append(String.format("%s %s - %d%n", Utils.getReactionInPos(place), score.getPerson().getName(), points));
		}
	...
	public static MessageEmbed votersToEmbededMessage(TierList tierList)
	{
		...
		for(Person person : tierList.getGroup().getPeople())
		{
			...
			for(Voter voter : voters)
			{
				...
				description += String.format("%s %s ", Utils.getReactionInPos(score.getScore()), voter.getName());
			}
