...
public class StartTierList implements Command
{
	...
	public void run(String[] args, Bot bot, MessageChannel channel, User user)
	{
		...
		List<User> voters = channel.getMessageById(id).complete().getReactions().get(0).getUsers().complete();
	...
	public MessageEmbed getHelpMessage()
	{
		...
		eb.addField("Example: ", getCommandName() + " Twice TWICE_NEW_MV https://www.youtube.com/watch?v=Fm5iP0S1z9w", false);
