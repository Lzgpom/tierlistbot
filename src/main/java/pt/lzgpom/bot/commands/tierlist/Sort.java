...
public class Sort implements Command {
    ...
    public void run(String[] args, Bot bot, MessageChannel channel, User user) {
        ...
        executor.submit(() ->
        {
            ...
            if (group == null) {
                channel.sendMessage("There is no tierlist going or the name of the group is invalid").queue();
                return;
            }

            channel.sendMessage(String.format("%s, the sort of the group %s has been started on your PMs.", user.getName(), group.getName())).queue();
            ...
            for (int i = 0; (i < people.size() - 1 && swap); i++)
            {
                ...
                for (int j = 0; j < people.size() - i - 1; j++)
                {
                    ...
                    if(decision == null) {
                        ...
                        while (true) {
                            try {
                                if ((decision = getDecisionFromMessage(privateChannel, message.getIdLong(), p1, p2)) != null)
                                    break;
            ...
            if(tierlist) {
                Message endMessage = privateChannel.sendMessage("Is this the tierlist you want?").complete();
     ...
     */
    private static Decision getDecisionFromMessage(MessageChannel channel, long messageId, Person p1, Person p2) {
    ...
    /**
     * This class is a Decision between two people.
     * It is used to know the author decision only between this two people.
     */
