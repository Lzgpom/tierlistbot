package pt.lzgpom.bot.commands.graph;
...
public class GraphCompare implements Command {
      ...
      if (args.length == 1) {
        ...
        for (int i = 1; i < args.length; i++) {
          if (tierlist.getVoterByName(args[1]) != null) {
            ...
            channel.sendMessage(String.format("%s didn't vote on this tierlist...", args[i]))
                .queue();
          }
        }
      }
      ...
      for (Person person : group.getPeople()) {
        ...
        for (String voterName : votersName) {
          ...
          if (voter == null) {
            ...
            channel.sendMessage(
                String.format("The user %s didnt vote in %s.", voterName, tierlist.getId()))
                .queue();
            return;
          }
  ...
  private LineChart createLineChart(List<Plot> lines, List<String> votersName, int max,
      TierList tierlist) {
    //Creates a list of String for the x axis.

    List<String> axisLabels = new ArrayList<>(votersName);
