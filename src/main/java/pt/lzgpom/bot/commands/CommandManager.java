...
public class CommandManager extends ListenerAdapter
{
	...
	public void onMessageReceived(MessageReceivedEvent event)
	{
		...
		{
			String[] args = event.getMessage().getContentDisplay().replaceFirst(Config.PREFIX, "").trim().split(" ");
			...
			for(Command command : commands)
			{
				for(String commandName : command.getCommands())
				{
					...
					{
						command.run(Arrays.copyOfRange(args, 1, args.length), bot, event.getChannel(), event.getAuthor());
					}
				}
			}
		}
