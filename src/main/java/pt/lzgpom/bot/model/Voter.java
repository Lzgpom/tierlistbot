...
public class Voter {
    ...
    for (Score vote : votes) {
      if (values.contains(vote.getScore())) {
        throw new IllegalArgumentException(String
            .format("%s ranked %s with the same score as someone else.", name,
                vote.getPerson().getName()));
      }
  ...
  /**
   * Checks if the number of votes is the same as the group size and if the votes have all members
   * of the group.
   */
