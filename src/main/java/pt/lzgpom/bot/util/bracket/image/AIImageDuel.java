...
public class AIImageDuel {
  ...
  public BufferedImage createImage() {
    int height = (MARGIN_PADDING_VERTICAL * 4) + (FONT_SIZE * 2) + USER_AVATAR_SIZE + (
        (CHALLENGER_IMAGE_SIZE + FONT_SIZE) * duel.getFirstChallenger().size())
        + (CHALLENGER_PADDING_VERTICAL * (duel.getFirstChallenger().size() - 1));

    int userWidth =
        (USER_AVATAR_SIZE * users.size()) + ((USER_AVATAR_PADDING * (users.size() - 1)) * 2)
            + COUNTERS_SEPARATION_DIST;
    int challengerWidth = (CHALLENGER_IMAGE_SIZE * 3) + (CHALLENGER_PADDING_HORIZONTAL * 2);

    int width = (MARGIN_PADDING_HORIZONTAL * 2) + ((userWidth > challengerWidth) ? userWidth
        : challengerWidth);
    ...
    for (int i = 0; i < duel.getFirstChallenger().size(); i++) {
      ...
      if (i1 != null) {
        ...
        g.drawImage(Utils.getImageSquared(i1)
                .getScaledInstance(CHALLENGER_IMAGE_SIZE, CHALLENGER_IMAGE_SIZE, Image.SCALE_SMOOTH),
            x, y + FONT_SIZE, null);
      }

      if (i2 != null) {
        g.drawString(c2.getName(),
            x + (CHALLENGER_IMAGE_SIZE * 2) + (CHALLENGER_PADDING_HORIZONTAL * 2),
            y + ((FONT_SIZE / 3) * 2));
        g.drawImage(Utils.getImageSquared(i2)
                .getScaledInstance(CHALLENGER_IMAGE_SIZE, CHALLENGER_IMAGE_SIZE, Image.SCALE_SMOOTH),
            x + (CHALLENGER_IMAGE_SIZE * 2) + (CHALLENGER_PADDING_HORIZONTAL * 2), y + FONT_SIZE,
            null);
      }
  ...
  private int drawCounters(Graphics2D g, Map<User, Integer> map, int x, int y) {
    for (User user : users) {
      Image avatar = Utils.getImageFromUrl(user.getAvatarUrl())
          .getScaledInstance(USER_AVATAR_SIZE, USER_AVATAR_SIZE, Image.SCALE_DEFAULT);
      g.drawImage(avatar, x, y, null);
      g.drawString("" + map.get(user), x + (USER_AVATAR_SIZE / 4) + (FONT_SIZE / 3),
          y + USER_AVATAR_SIZE + FONT_SIZE);
