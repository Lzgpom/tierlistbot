...
public class Utils {
  ...
  /**
   * Creates the teams given the challengers, team size and number of teams. If there are not enough
   * different challengers for all teams, it duplicates some of them.
   *
   ...
   */
  public static List<List<Challenger>> createTeams(List<Challenger> challengers, int sizeTeam,
      int numTeam) {
  ...
  /**
   * Chooses randomly a {@link Challenger} from the list and removes it from the list.
   *
  ...
  private static BufferedImage getImageFromSearch(String query) {
  ...
  /**
   * From the image of the {@link Challenger} from saved files. If not found google searches for the
   * {@link Challenger}, saves it and then returns it.
   *
  ...
  public static BufferedImage getImage(Challenger challenger) {
    try {
      return getImageSquared(
          ImageIO.read(new File(DATABASE + IMAGES + challenger.getName() + ".jpg")));
    } catch (IOException e) {
      BufferedImage image = getImageFromSearch(
          challenger.getName() + " " + challenger.getExtraInfo());
