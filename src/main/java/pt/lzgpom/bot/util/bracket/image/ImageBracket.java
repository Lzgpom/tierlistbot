...
public class ImageBracket {
  ...
  public BufferedImage createImage() {
    ...
    int height =
        (MARGIN_PADDING_VERTICAL * 2) + (lastPartSize * ImageDuel.DUEL_HEIGHT) + ((lastPartSize - 1)
            * DUELS_PADDING_VERTICAL);
    int width = (MARGIN_PADDING_HORIZONTAL * 2) + (bracket.size() * ImageDuel.DUEL_WIDTH) + (
        (bracket.size() - 1) * DUELS_PADDING_HORIZONTAL);
    ...
    if (bracket.size() > 1 && bracket.get(1).size() == 2) {
      imageDuels.add(new ImageDuel(bracket.get(0).get(1),
          width - MARGIN_PADDING_HORIZONTAL - ImageDuel.DUEL_WIDTH,
          height / 2 + DUEL_LOOSER_FINAL_PADDING + ImageDuel.DUEL_HEIGHT));

      int necessary =
          (height / 2) + DUEL_LOOSER_FINAL_PADDING + ImageDuel.DUEL_HEIGHT + (ImageDuel.DUEL_HEIGHT
              / 2) + MARGIN_PADDING_VERTICAL;
      ...
      if (!part.isDummy()) {
        ...
        if (part.getMidX() + ImageDuel.DUEL_WIDTH < width - MARGIN_PADDING_HORIZONTAL) {
          ...
          if (part.getDuel().getWinner() != null) {
            color = (part.getDuel().getCounterUser() != null) ? userColors
                .get(part.getDuel().getCounterUser()) : userColors.get(part.getDuel().getUser());
          }

          g.setColor(color);

          //Front part
          g.drawLine(part.getMidX() + ImageDuel.DUEL_WIDTH + LINE_THICKNESS, part.getMidY(),
              part.getMidX() + ImageDuel.DUEL_WIDTH + (DUELS_PADDING_HORIZONTAL / 2),
              part.getMidY());
          ...
          GradientPaint gradient = new GradientPaint(startX, startY, color, startX,
              startY + Math.abs(part.getMidY() - part.getNext().getMidY()), Color.WHITE);

          if (startY > part.getNext().getMidY()) {
            startY = part.getNext().getMidY();
            gradient = new GradientPaint(startX,
                startY + Math.abs(part.getMidY() - part.getNext().getMidY()), color, startX, startY,
                Color.WHITE);
          }

          g.setPaint(gradient);
          g.fill(new Rectangle2D.Double(startX, startY, LINE_THICKNESS,
              Math.abs(part.getMidY() - part.getNext().getMidY())));

          //Back part of the next.
          g.setColor(Color.WHITE);
          g.drawLine(part.getNext().getMidX() - (DUELS_PADDING_HORIZONTAL / 2),
              part.getNext().getMidY(), part.getNext().getMidX(), part.getNext().getMidY());
        }
      }
    }
